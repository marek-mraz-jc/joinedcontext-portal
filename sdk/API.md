...
`EntityHistory` shows one attribute's temporal answer: a numeric series as a line in the design tokens' first chart colour over a time axis in local time and a value axis naming `unit`, each point giving its time and value on hover, and a table of every value for a screen reader. `heading` is the panel's heading in the App's words (the default is the title and the attribute; the entity's id is never shown), `locale` the language times and numbers are written in (the page's by default) and `fractionDigits` the attribute's precision (2 by default). Copy as CSV keeps the raw values.
