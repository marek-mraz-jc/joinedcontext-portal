...
export const BASE = "http://portal.test/apps/helsinki-events/";
...
export async function serve(page: Page): Promise<Served> {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (url.pathname.startsWith(`/apps/helsinki-events/api/endpoint/${SLUG}/`)) {
      const body = route.request().postData();
      const path = url.pathname.slice("/apps/helsinki-events".length) + url.search;
    ...
    if (!url.pathname.startsWith("/apps/helsinki-events/")) return route.fulfill({ status: 404, body: "" });
    const file = normalize(url.pathname.slice("/apps/helsinki-events/".length) || "index.html");
