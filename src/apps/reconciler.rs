...
fn endpoint(
    ...
    let (audience, allowed_projects) = match spec.visibility {
        ...
        // Who holds a role is checked per request on this endpoint; the subjects are people and
        // groups of the organization (AP-91), so the organization is who it admits.
        AppVisibility::Organization | AppVisibility::Roles => ("organization", Vec::new()),
